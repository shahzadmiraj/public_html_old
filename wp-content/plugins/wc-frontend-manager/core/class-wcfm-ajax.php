...
class WCFM_Ajax {
  ...
  public function wcfm_modify_order_status() {
  	global $WCFM;
  	if(isset($_POST['wcfm_orders_manage_form'])){
  		$searcharray=array();
  		parse_str($_POST['wcfm_orders_manage_form'], $searcharray);
  		if(array_key_exists('billing_order_id',$searcharray)){
					// start address billing
					$isChangeAddress = false;
					$order_id = $searcharray['billing_order_id'];
					...
					$_billing_address_1 = get_post_meta( $order_id, '_billing_address_1', true );
					$_billing_address_2 = get_post_meta( $order_id, '_billing_address_2', true );
					...
					$_billing_address_index = get_post_meta( $order_id, '_billing_address_index', true );
					$_billing_email = get_post_meta( $order_id, '_billing_email', true );

					if($searcharray['billing_first_name'] != $_billing_first_name ){
						update_post_meta( $order_id, '_billing_first_name', $searcharray['billing_first_name'] );
						$isChangeAddress = true;
					}

					if($searcharray['billing_last_name'] != $_billing_last_name){
						update_post_meta( $order_id, '_billing_last_name', $searcharray['billing_last_name'] );
						$isChangeAddress = true;
					}

					if($searcharray['billing_phone'] != $_billing_phone ){
						update_post_meta( $order_id, '_billing_phone', $searcharray['billing_phone'] );
						$isChangeAddress = true;
					}

					if($searcharray['billing_address_1'] != $_billing_address_1 ){
						update_post_meta( $order_id, '_billing_address_1', $searcharray['billing_address_1'] );
						$isChangeAddress = true;
					}

					if($searcharray['billing_address_2'] != $_billing_address_2){
						update_post_meta( $order_id, '_billing_address_2', $searcharray['billing_address_2'] );
						$isChangeAddress = true;
					}

					if($searcharray['billing_country'] != $_billing_country ){
						update_post_meta( $order_id, '_billing_country', $searcharray['billing_country'] );
						$isChangeAddress = true;
					}

					if($searcharray['billing_city'] != $_billing_city ){
						update_post_meta( $order_id, '_billing_city', $searcharray['billing_city'] );
						$isChangeAddress = true;
					}

					if($searcharray['billing_state'] != $_billing_state){
						update_post_meta( $order_id, '_billing_state', $searcharray['billing_state'] );
						$isChangeAddress = true;
					}

					if($searcharray['billing_postcode'] != $_billing_postcode){
						update_post_meta( $order_id, '_billing_postcode', $searcharray['billing_postcode'] );
						$isChangeAddress = true;
					}

					if($isChangeAddress){
							$_billing_address_index_current_with_format = $_billing_first_name.' '.$_billing_last_name.' '.$_billing_address_1.' '.$_billing_address_2.' '.$_billing_city.' '.$_billing_state.' '.$_billing_country.' '.$_billing_email.' '.$_billing_phone;
						$order = wc_get_order( $order_id );
						// Add Order Note for Log
						$user_id = apply_filters( 'wcfm_current_vendor_id', get_current_user_id() );
						$shop_name =  get_user_by( 'ID', $user_id )->display_name;
						...
						$wcfm_messages = sprintf( __( 'Billing information updated to <b>%s</b> by <b>%s</b>', 'wc-frontend-manager' ),get_post_meta( $order_id, '_billing_address_index', true ) , $shop_name );
						$is_customer_note = apply_filters( 'wcfm_is_allow_order_update_note_for_customer', '1' );

						$comment_id = $order->add_order_note( $wcfm_messages, $is_customer_note);
						if( wcfm_is_vendor() ) { add_comment_meta( $comment_id, '_vendor_id', $user_id ); }

						update_post_meta( $order_id, '_billing_address_index', $_billing_address_index_current_with_format );


					}
					//end start address billing

  		}
  		else if(array_key_exists('shiping_address_order_id',$searcharray)){
  				//start shipping address

					$isChangeAddress = false;
					$order_id = $searcharray['shiping_address_order_id'];
					...
					//$_shipping_phone = get_post_meta( $order_id, '_shipping_phone', true );
					...
					$_shipping_address_index = get_post_meta( $order_id, '_shipping_address_index', true );
					$_shipping_email = get_post_meta( $order_id, '_shipping_email', true );

					if($searcharray['shipping_first_name'] != $_shipping_first_name ){
						update_post_meta( $order_id, '_shipping_first_name', $searcharray['shipping_first_name'] );
						$isChangeAddress = true;
					}

					if($searcharray['shipping_last_name'] != $_shipping_last_name){
						update_post_meta( $order_id, '_shipping_last_name', $searcharray['shipping_last_name'] );
						$isChangeAddress = true;
					}

					// if($searcharray['shipping_phone'] != $_shipping_phone ){
					// 	update_post_meta( $order_id, '_shipping_phone', $searcharray['shipping_phone'] );
					// 	$isChangeAddress = true;
					// }

					if($searcharray['shipping_address_1'] != $_shipping_address_1 ){
						update_post_meta( $order_id, '_shipping_address_1', $searcharray['shipping_address_1'] );
						$isChangeAddress = true;
					}

					if($searcharray['shipping_address_2'] != $_shipping_address_2){
						update_post_meta( $order_id, '_shipping_address_2', $searcharray['shipping_address_2'] );
						$isChangeAddress = true;
					}

					if($searcharray['shipping_country'] != $_shipping_country ){
						update_post_meta( $order_id, '_shipping_country', $searcharray['shipping_country'] );
						$isChangeAddress = true;
					}

					if($searcharray['shipping_city'] != $_shipping_city ){
						update_post_meta( $order_id, '_shipping_city', $searcharray['shipping_city'] );
						$isChangeAddress = true;
					}

					if($searcharray['shipping_state'] != $_shipping_state){
						update_post_meta( $order_id, '_shipping_state', $searcharray['shipping_state'] );
						$isChangeAddress = true;
					}

					if($searcharray['shipping_postcode'] != $_shipping_postcode){
						update_post_meta( $order_id, '_shipping_postcode', $searcharray['shipping_postcode'] );
						$isChangeAddress = true;
					}

					if($isChangeAddress){
							$_shipping_address_index_current_with_format = $_shipping_first_name.' '.$_shipping_last_name.' '.$_shipping_address_1.' '.$_shipping_address_2.' '.$_shipping_city.' '.$_shipping_state.' '.$_shipping_country.' '.$_shipping_email;
						$order = wc_get_order( $order_id );
						// Add Order Note for Log
						$user_id = apply_filters( 'wcfm_current_vendor_id', get_current_user_id() );
						$shop_name =  get_user_by( 'ID', $user_id )->display_name;
						...
						$wcfm_messages = sprintf( __( 'shipping information updated to <b>%s</b> by <b>%s</b>', 'wc-frontend-manager' ),get_post_meta( $order_id, '_shipping_address_index', true ) , $shop_name );
						$is_customer_note = apply_filters( 'wcfm_is_allow_order_update_note_for_customer', '1' );

						$comment_id = $order->add_order_note( $wcfm_messages, $is_customer_note);
						if( wcfm_is_vendor() ) { add_comment_meta( $comment_id, '_vendor_id', $user_id ); }

						update_post_meta( $order_id, '_shipping_address_index', $_shipping_address_index_current_with_format );
					}
					//end shipping address
  		}

  		else if(array_key_exists('extra_edit_order_id',$searcharray)){
  				//start order edit

  				$isChangeAddress = false;
					$order_id = $searcharray['extra_edit_order_id'];
					$_transaction_id = get_post_meta( $order_id, '_transaction_id', true );
					$_wcpdf_invoice_notes = get_post_meta( $order_id, '_wcpdf_invoice_notes', true );
					$_wcfmd_delvery_times = get_post_meta( $order_id, '_wcfmd_delvery_times', true );


					$delivery_time = "";
					$delivery_date = "";
					if(is_array($_wcfmd_delvery_times)){
						$wcfmd_delvery_times = $_wcfmd_delvery_times;
					...
					if(($searcharray['current_delivery_date']!=$delivery_date)||($searcharray['current_delivery_time']!=$delivery_time)){
						$isChangeAddress = true;

						$date = new DateTime($searcharray['current_delivery_date'].' '.$searcharray['current_delivery_time']);
						$deliveryDateAndTime = array();
						$deliveryDateAndTime[0] = $date->getTimestamp();
						update_post_meta( $order_id, '_wcfmd_delvery_times',$deliveryDateAndTime);

						$messageComment ="Delivery Time changed: ".$delivery_date." | ".$delivery_time." ";
					}

					if($searcharray['current_invoice_note'] != $_wcpdf_invoice_notes){
					update_post_meta( $order_id, '_wcpdf_invoice_notes', $searcharray['current_invoice_note'] );

					$isChangeAddress = true;

					$messageComment.=", Invoice note changed: ".$_wcpdf_invoice_notes;
					}
					...
					if($isChangeAddress){
						...
						$wcfm_messages = sprintf( __( 'information updated to <b>%s</b> by <b>%s</b>', 'wc-frontend-manager' ),$messageComment , $shop_name );
						$is_customer_note = apply_filters( 'wcfm_is_allow_order_update_note_for_customer', '1' );
  		...
  		else if(array_key_exists('add_more_products_order_id',$searcharray)){

  			//start adding new more products
  			$order_id = $searcharray['add_more_products_order_id'];
	  		$associate_products    = $searcharray['associate_products'];
		  	$product_ids           = array(); 
		  	$current_user_id = apply_filters( 'wcfm_current_vendor_id', get_current_user_id() );
		  	$vendor_id = '';

		  	printf($associate_products);
			  	if( wcfm_is_vendor() ) {
  			...
  			//end new more products

  		}else if(array_key_exists('remove__products_from_order',$searcharray)){
  			$item = WC_Order_Factory::get_order_item( absint( $searcharray['item_id'] ) );
  			$item->delete();
  			$order_id = $searcharray['order_id'];
  			$order = wc_get_order( $order_id );
  			$order->save();
				$order->update_taxes();
				$order->calculate_totals();
  		}else if(array_key_exists('wc_coupon_list_order_id',$searcharray)){
  			$order_id = $searcharray['wc_coupon_list_order_id'];
  			$order = wc_get_order( $order_id );
   			$order->remove_coupon($searcharray['coupon_code_title']);
  			$order->save();
				$order->update_taxes();
				$order->calculate_totals();
  		}
  		else if(array_key_exists('add_shipping_method_order_id',$searcharray)){
  			$order_id = $searcharray['add_shipping_method_order_id'];
  			$order = wc_get_order( $order_id );
  			$wcfm_orders_manage_form_data = $searcharray;
  			$quantity = 0;
  			if($wcfm_orders_manage_form_data['wcfm_om_shipping_quantity']>0){
  				$quantity = $wcfm_orders_manage_form_data['wcfm_om_shipping_quantity'];
  			}
   		  ...
   		  if( isset( $wcfm_orders_manage_form_data['wcfm_om_shipping_method'] ) && !empty( $wcfm_orders_manage_form_data['wcfm_om_shipping_method'] ) ) {
					...
					if(trim($wcfm_orders_manage_form_data['wcfm_om_shipping_title'])!=""){
						$titleAndMethod=$wcfm_orders_manage_form_data['wcfm_om_shipping_title'];
						$titleAndMethod .=" | Method: ".$wcfm_orders_manage_form_data['wcfm_om_shipping_method'];
						$shipping_item->set_method_title($titleAndMethod);
					}
   		  ...
   		  $order->save();
				$order->update_taxes();
				$order->calculate_totals();
  		}else if(array_key_exists('remove__shipping_from_order',$searcharray)){
  			$order_id = $searcharray['order_id'];
  			$order = wc_get_order( $order_id );
  		...
  		unset($_POST['wcfm_orders_manage_form']);
  		die();
